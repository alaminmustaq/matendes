...
} from "../services/punishmentGeneralApi";
...
export const usePunishmentGeneral = () => {
    ...
    const [createPunishmentGeneral] = useCreatePunishmentGeneralMutation();
    const [updatePunishmentGeneral] = useUpdatePunishmentGeneralMutation();
    const [deletePunishmentGeneral] = useDeletePunishmentGeneralMutation();
    const [deleteGroupPunishmentGeneral] =
        useDeleteGroupPunishmentGeneralMutation();
    // For Filters
    ...
    const {
        data: punishmentGeneralData,
        refetch,
        isFetching,
    } = useFetchPunishmentGeneralQuery({ params: queryParams });
    ...
    const punishmentGeneralState = {
        data: punishmentGeneralData?.data?.items || [],
        form: {
            ...form,
            fields: fieldArray,
        ...
        refetch,
        pagination: punishmentGeneralData?.data?.pagination || {},
        isFetching,
    };
    ...
    const actions = {
        ...
        onCreate: async (data) => {
            ...
            if (form.watch("model_for") == "delete_group_punishment") {
                try {
                    if (
                        !confirm(
                            "Are you sure you want to delete this punishment/general?",
                        )
                    )
                        return;
                    ...
                    const response =
                        await deleteGroupPunishmentGeneral(
                            preparedData,
                        ).unwrap();

                    if (response) {
                        toast.success(
                            "Group PunishmentGeneral deleted successfully",
                        );
                        refetch();
                        formReset(form);
                    ...
                    console.error("Delete punishment/general error:", error);

                    const message =
                        error?.data?.message ||
                        error?.message ||
                        "Something went wrong while deleting punishment/general.";
            ...
            // Create
                ...
                try {
                    ...
                    // Call the API
                    const response =
                        await createPunishmentGeneral(preparedData).unwrap();
                    ...
                    if (response) {
                        ...
                        toast.success(
                            skippedCount > 0
                                ? `PunishmentGeneral created successfully. ${payload.processed_count || 0} processed, ${skippedCount} skipped.`
                                : "PunishmentGeneral created successfully",
                        );
                    ...
                    toast.error("Failed to create punishment/general");
                }
            }
        },
        ...
        onEdit: (item) => {
            ...
            form.reset({
                ...
                type: item.allowance_type?.type || null,
                punishment_type_id:
                    holidayTypeSearchTemplate(
                        item?.punishment_type ? [item?.punishment_type] : [],
                    )?.at(0) ?? null,
                scope_type: item.project_id ? "project" : "company",
                amount_type: item.amount_type,
                effective_date: item.effective_date,
                punishment_month: item.punishment_month,
                branch_id:
                    branchSearchTemplate(
        ...
        onUpdate: async (data) => {
            ...
            try {
                ...
                const preparedData = {
                    ...
                    allowance_type_id:
                        normalized.allowance_type_id?.value ??
                        normalized.allowance_type_id ??
                        null,
                    branch_id:
                        normalized.branch_id?.value ??
                    ...
                    department_id: normalized.department_id
                        ...
                        : null,
                    job_position_id:
                        normalized.job_position_id?.value ??
                        normalized.job_position_id ??
                        null,
                    project_id:
                ...
                // 🔢 FIX TYPES
                // ================================
                preparedData.amount = Number(preparedData.amount) || null;

                // status → backend expects number
                if (preparedData.status === "active") preparedData.status = 1;
                else if (preparedData.status === "inactive")
                ...
                // 🚀 API CALL
                // ================================
                const response = await updatePunishmentGeneral({
                    id: normalized.id,
                    ...preparedData,
                }).unwrap();
        ...
        onDelete: async (id) => {
            try {
                if (
                    !confirm("Are you sure you want to delete this punishment?")
                )
                    return;

                const response = await deletePunishmentGeneral(id).unwrap();

                if (response) {
                    toast.success("PunishmentGeneral deleted successfully");
                    refetch();
                }
            } catch (error) {
        ...
        onApproveSingleApplication: async () => {
            form.reset({
                openModel: true,
                model_for: "approve_single_punishment",
                ...defaultValue,
            });
        },
        onCloseSkippedModal: () => {
