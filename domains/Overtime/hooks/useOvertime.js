...
export const useOvertime = () => {
    ...
    const {
        ...
        refetch,
        isFetching,
    } = useFetchOvertimeQuery(queryParams);
    ...
    const overtimeState = {
        data: overtimeData?.data?.items || [], 
        form: {
            ...form,
            fields: fieldArray,
        },
        refetch,
        pagination: overtimeData?.data?.pagination || {}, 
        isFetching,
    };
    ...
    const actions = {
        ...
        onCreate: async (data) => {
            try {
                ...
                // 🔥 employee_ids now acts as an EXCLUSION list in the backend
                const finalPayload = {
                    ...normalized,
                ...
                await createOvertime(finalPayload).unwrap();

                toast.success("Overtime settings processed successfully");
                refetch();
                formReset(form);
                form.setValue("openModel", false);

            } catch (apiErrors) {
                console.error("Create overtime error:", apiErrors);
                handleServerValidationErrors(apiErrors, form.setError);
                toast.error("Failed to create overtime records");
            }
        },
        ...
        onEdit: (item) => {
         ...
         onUpdate: async (data) => {
            try {
                ...
                console.error("Update overtime error:", apiErrors);
                handleServerValidationErrors(apiErrors, form.setError);
                toast.error(apiErrors?.data?.message || "Failed to update overtime");
            }
        },
