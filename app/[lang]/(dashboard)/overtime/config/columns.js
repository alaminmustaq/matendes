...
const overtimeColumns = (actions) => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className={`px-2 py-0.5 rounded-full ${isActive ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-700"}`}>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <TableActions
        ...
        label="Actions"
        items={[
          { label: "View", onClick: actions?.onView, permission: "manual-attendance" },
          { label: "Edit", onClick: actions?.onEdit, permission: "manual-attendance" },
          { label: "Delete", onClick: actions?.onDelete, danger: true, passId: true, permission: "manual-attendance" },
        ]}
      />
    ),
  },
